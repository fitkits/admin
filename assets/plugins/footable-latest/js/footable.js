...
(function($, F) {
	...
	F.checkFnValue = function(self, value, def) {
		...
		return F.is.fn(value)
			? wrap(self, value, def)
			: F.is.type(value, 'string') ? wrap(self, F.getFnPointer(value), def) : def;
	};
})(jQuery, FooTable);
(function($, F) {
		...
		/** @lends FooTable.Cell */ {
			...
			define: function(valueOrElement) {
				...
				this.value = this.column.parser.call(
					this.column,
					F.is.jq(this.$el) ? this.$el : hasOptions ? valueOrElement.value : valueOrElement,
					this.ft.o
				);
				...
				this.classes =
					F.is.jq(this.$el) && this.$el.attr('class')
						...
						: F.is.array(this.o.classes)
							? this.o.classes
							: F.is.string(this.o.classes) ? this.o.classes.match(/\S+/g) : [];
				/**
				...
				this.style =
					F.is.jq(this.$el) && this.$el.attr('style')
						...
						: F.is.hash(this.o.style)
							? this.o.style
							: F.is.string(this.o.style) ? F.css2json(this.o.style) : {};
			},
			/**
		...
		/** @lends FooTable.Column */ {
			...
			define: function(definition) {
				...
				this.style = F.is.hash(definition.style)
					? definition.style
					: F.is.string(definition.style) ? F.css2json(definition.style) : {};
				/**
				...
				this.classes = F.is.array(definition.classes)
					? definition.classes
					: F.is.string(definition.classes) ? definition.classes.match(/\S+/g) : [];
			...
			$create: function() {
				...
				this.$details = $('<tr/>', { class: 'footable-detail-row' }).append(
					$('<td/>', { colspan: this.ft.columns.visibleColspan }).append(
						$('<table/>', { class: 'footable-details ' + this.ft.classes.join(' ') }).append('<tbody/>')
					)
				);
		...
		/** @lends FooTable.Table */ {
			...
			construct: function(element, options, ready) {
				...
				this.components = F.components.load(
					F.is.hash(this.data.components) ? this.data.components : this.o.components,
					this
				);
				/**
...
(function($, F) {
	...
	F.DateColumn = F.Column.extend(
		/** @lends FooTable.DateColumn */ {
			 ...
			 */
			formatter: function(value, options, rowData) {
				return F.is.object(value) && F.is.boolean(value._isAMomentObject) && value.isValid()
		...
		/** @lends FooTable.Columns */ {
			...
			parse: function(data) {
				...
				return $.Deferred(function(d) {
					...
					if ($header.length > 0) {
						var virtual =
							$header.parent().is('tbody') && $header.children().length == $header.children('td').length;
					...
					if (F.is.array(self.o.columns) && !F.is.emptyArray(self.o.columns)) {
						...
						self.o.columns.then(
							...
							function(xhr) {
								d.reject(
									Error('Columns ajax request error: ' + xhr.status + ' (' + xhr.statusText + ')')
								);
							}
						);
					} else {
	...
	F.Filter = F.Class.extend(
		/** @lends FooTable.Filter */ {
			...
			construct: function(name, query, columns, space, connectors, ignoreCase, hidden) {
				 ...
				 */
				this.query =
					query instanceof F.Query ? query : new F.Query(query, this.space, this.connectors, this.ignoreCase);
				/**
	...
	F.Filtering = F.Component.extend(
		/** @lends FooTable.Filtering */ {
			...
			$create: function() {
				...
				var $form_grp = $('<div/>', { class: 'form-group footable-filtering-search' }).append(
						$('<label/>', { class: 'sr-only', text: 'Search' })
					),
			...
			_ensure: function(filter, selectedColumns) {
				if (
					...
					filter.space =
						F.is.string(filter.space) && (filter.space === 'AND' || filter.space === 'OR')
							? filter.space
							: this.space;
					filter.query = F.is.string(filter.query)
						? new F.Query(filter.query, filter.space, filter.connectors, filter.ignoreCase)
						: filter.query;
		...
		/** @lends FooTable.Sorting */ {
			...
			draw: function() {
				...
				$sortable
					.not($active)
					.children('.fooicon')
					.addClass('fooicon-sort');
				$active
					.addClass(self.column.direction == 'DESC' ? 'footable-desc' : 'footable-asc')
					.children('.fooicon')
					.addClass(self.column.direction == 'DESC' ? 'fooicon-sort-desc' : 'fooicon-sort-asc');
			},
				...
				return !(
					...
					(this.column.name === this.initial.column &&
						(this.column.direction === this.initial.direction ||
							(this.initial.direction === null && this.column.direction === 'ASC')))
				);
			},
			/**
...
(function($, F) {
	F.HTMLColumn.extend('__sorting_define__', function(definition) {
		this._super(definition);
		this.sortUse =
			F.is.string(definition.sortUse) && $.inArray(definition.sortUse, ['html', 'text']) !== -1
				? definition.sortUse
				: 'html';
	});
	...
	F.Paging = F.Component.extend(
		/** @lends FooTable.Paging */ {
			...
			_setNavigation: function(active) {
				...
				if (
					(this.$pagination.children('li.footable-page.visible:last').data('page') || this.limit) ==
					this.total
				) {
				...
				this.ft.raise('preinit.ft.editing', [data]).then(
					function() {
						...
						self.column = new F.EditingColumn(
							self.ft,
							self,
							$.extend(true, {}, self.column, data.editingColumn, { visible: self.alwaysShow })
						);
...
(function($, F) {
	F.EditingColumn = F.Column.extend(
		/** @lends FooTable.EditingColumn */ {
			...
			$create: function() {
				(this.$el =
					!this.virtual && F.is.jq(this.$el) ? this.$el : $('<th/>', { class: 'footable-editing' })).html(
					this.title
				);
			},
			/**
